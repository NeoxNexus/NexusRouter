...
1. **双轨驱动 (SDD+TDD)**：必须先明确核心设计与数据流，写功能前**绝对优先编写测试**并覆盖异常边界。（详见 `.Codex/rules/workflow.md`）
2. **严守工作流与门禁**：`ROADMAP.md` 是唯一真相源，任务必须同步。提交前必须过 `typecheck`, `build` 与 `test` 100% 绿灯。（详见 `.Codex/rules/workflow.md`）
3. **架构与性能底线**：保持极致毫秒级性能，保证**绝对向后兼容**（零感知升级），合理借用策略/插件等设计模式。（详见 `.Codex/rules/architecture.md`）
4. **对等辩驳 (Critical!)**：指令非圣旨！若发现人类要求有漏洞或技术债务，务必质疑、列出优缺点并**强力提出更优替代方案**。（详见 `.Codex/rules/agent-behavior.md`）
5. **产品级交付**：输出必须具备极客视觉美感，注重前沿系统兼容，使用专业架构图（Mermaid）与精准排版。（详见 `.Codex/rules/agent-behavior.md`）
...
- **COMPLEX** — 代码生成、多步分析 → Codex Sonnet
...
tiers:
  ...
  MEDIUM:
    ...
    fallback: []
  COMPLEX:
    primary: anthropic/Codex-sonnet-4-20250514
    fallback: []
  REASONING:
