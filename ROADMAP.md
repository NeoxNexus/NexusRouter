...
## 📍 当前状态（2026-09-01 二次校准）

- **版本** 0.12.7 · **门禁** `npm test` **748 通过 + 1 预期失败**（36 文件）· typecheck / lint 全绿
- **当前焦点**：Phase 4 —— 分类器正确性（Benchmark）
- **下一步（按依赖排序）**：
  1. **4.4 标注**：第二批日志（8/28~9/1，1114 条）字段已齐，去重后 **236 个唯一输入**可开工；其中 67 条 D-009 污染样本单独分层
  2. **4.6 删 Layer 1 置信门**：`heuristicThreshold = 0.92` 两批真实流量均 0 命中，结论已定（承认两层架构），随 4.4 基线同批落地
  3. **D-005 档位语义定案**：陈旧轮次沿用首轮 tier 还是纳入 tool_result 摘要 —— 定案后重攒流量出正式基线（4.4/4.5 的前置）
  4. **3.9 文档表述收尾**：`CLAUDE.md`（仍写「14 维」）、`docs/architecture.md`、`docs/features.md`、`docs/routing-profiles.md`、`src/adapter/` 两处注释
...
- [x] **5.6** 接入 `Logger` / `Stats` / `Report` —— **Savings Ledger 省钱记账体系** ✅ **核心全部完成**（方案：[`docs/plans/2026-08-19-savings-ledger-design.md`](docs/plans/2026-08-19-savings-ledger-design.md)；5.6.1–5.6.7 交付明细见下方折叠块）
...
<details>
<summary>5.6 省钱记账体系交付明细（5.6.1–5.6.7 全部完成，2026-08-19 ~ 08-26）</summary>
