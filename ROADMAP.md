...
> D-001 阻塞 Phase 3（能力侧遗留）。D-002 已在 Phase 4.3 修复，回归集锁定。
...
- **基线翻转**：启发式基线由 SIMPLE 改为 MEDIUM，SIMPLE 改为需要正面证据（`isTrivialQuery`：疑问句或文本操作，且不碰项目物件/动手动词/camelCase 标识符）。原设计「先假设最简单再无条件累加」正是棘轮的成因。
